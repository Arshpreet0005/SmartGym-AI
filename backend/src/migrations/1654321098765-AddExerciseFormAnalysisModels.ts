...
export class AddExerciseFormAnalysisModels1654321098765 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        const exerciseFormAnalysisExists = await queryRunner.hasTable("exercise_form_analysis");
        if (!exerciseFormAnalysisExists) {
            await queryRunner.query(`
                CREATE TABLE IF NOT EXISTS "exercise_form_analysis" (
                    ...
                    "repetitionsDetected" integer NOT NULL,
                    "goodFormRepetitions" integer NOT NULL,
                    "video_media_id" uuid,
                    "overallScore" integer NOT NULL,
                    "summary" text NOT NULL,
                    "isViewed" boolean NOT NULL DEFAULT false,
                    "isDismissed" boolean NOT NULL DEFAULT false,
                    "metric_tracking_id" uuid,
                    "analysisData" jsonb,
                    "metadata" jsonb,
                    "created_at" TIMESTAMP NOT NULL DEFAULT now(),
                    "updated_at" TIMESTAMP NOT NULL DEFAULT now(),
                    CONSTRAINT "PK_exercise_form_analysis" PRIMARY KEY ("id")
                )
            `);
            ...
            await queryRunner.query(`
                DO $$
                BEGIN
                    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'bodypositionstate') THEN
                        CREATE TYPE "bodypositionstate" AS ENUM('CORRECT', 'NEEDS_ADJUSTMENT', 'INCORRECT');
                    END IF;
                    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'feedbackseverity') THEN
                        CREATE TYPE "feedbackseverity" AS ENUM('INFO', 'SUGGESTION', 'WARNING', 'CRITICAL');
                    END IF;
                END
                $$;
            `);
            ...
            await queryRunner.query(`
                CREATE TABLE IF NOT EXISTS "form_correction_points" (
                    "id" uuid NOT NULL DEFAULT uuid_generate_v4(),
                    "analysis_id" uuid NOT NULL,
                    "timeOffset" double precision NOT NULL,
                    "bodyPart" character varying(50) NOT NULL,
                    "state" "bodypositionstate" NOT NULL DEFAULT 'NEEDS_ADJUSTMENT',
                    "detectedAngle" double precision,
                    "targetAngle" double precision,
                    "severity" "feedbackseverity" NOT NULL DEFAULT 'SUGGESTION',
                    "message" text NOT NULL,
                    "correctionInstruction" text NOT NULL,
                    "image_media_id" uuid,
                    "metadata" jsonb,
                    "created_at" TIMESTAMP NOT NULL DEFAULT now(),
                    CONSTRAINT "PK_form_correction_points" PRIMARY KEY ("id")
                )
            `);
            ...
            await queryRunner.query(`
                ...
                BEGIN
                    IF NOT EXISTS (
                        SELECT 1 FROM pg_constraint WHERE conname = 'FK_form_correction_points_analysis'
                    ) THEN
                        ALTER TABLE "form_correction_points"
                        ADD CONSTRAINT "FK_form_correction_points_analysis"
